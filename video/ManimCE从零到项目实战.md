...
| 218–238 | `make_stack_example` | 制作开场使用的单根栈模型图示 |
| 241–365 | `BirdBoard` | 把棋盘状态转换为可动画对象 |
| 368–438 | `SnapshotCard` | 制作 Beam Search 的候选状态缩略图 |
| 441–473 | 评分与下一层搜索 | 生成真实候选状态并保留前三名 |
| 476–521 | UI 辅助组件 | 徽章和最终对比卡片 |
| 524 行以后 | `BeamSearchCore` | 编排完整的 75 秒时间线 |
...
| 阶段 | 画面内容 |
|---|---|
| 问题建模 | 将树枝表示成先进后出的栈，标出栈底和栈顶 |
| 移动规则 | 高亮并移动栈顶连续同类鸟 |
| 目标状态 | 展示“同类鸟树枝”和“空树枝”两种完成状态 |
| 教学残局 | 标题、六根树枝和小鸟出现 |
| 束宽 1 | 强调“看起来最好”的移动 |
