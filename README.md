...
重复点击中央空白区域；批次内最多连续执行八次移动后校验实际棋盘。批次结束先用单张截图
严格匹配预期的可见树枝和鸟类结构，命中时直接重新规划；不匹配时会复用该截图继续做稳定
截图校验。树枝消除会立即结束当前批次，因此布局变化后始终会重新截图识别；仅在首次启动、
点击未生效或中断恢复时清除残留选择。`analyze`
